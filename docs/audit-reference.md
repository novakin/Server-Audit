...
Home, key directory and key file ownership/modes are checked for unexpected owners and group/other write access. A symbolic link's mode bits do not establish target access: the link's ownership is still checked, but its target permissions remain explicitly Unknown, including for dangling links. The permission lookup uses `lstat()` without dereferencing the final link; this is not an all-ancestor or race-proof traversal guarantee. These checks do not cover ACLs or every ancestor directory. Key-content inspection still skips symlink paths, non-regular files and files above 1 MiB with an unknown result. Private key files and password hashes are not read. See [permission evidence](report-format.md#account-permission-evidence) for field meanings.
